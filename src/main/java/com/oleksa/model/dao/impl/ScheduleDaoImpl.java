    ...
    public PaginationResult<Schedule> findPage(int page, int itemsOnPage) {
        ...
        try(Connection connection = dataSource.getConnection()) {
            ...
            try (PreparedStatement statement = connection.prepareStatement(SC_SELECT_LIMIT.getValue());
                ...
                try(ResultSet resultSet = statement.executeQuery();) {
                    while(resultSet.next()) {
                        Schedule schedule = JdbcMapperImpl.mapToScheduleWithUser(resultSet);
                        Integer scheduleId = schedule.getId();
                        if (Objects.isNull(result.putIfAbsent(scheduleId, schedule))) {
                            schedule.initFreeHours();
                        } else {
                            schedule = result.get(scheduleId);
                        }
                        Time time = resultSet.getTime(SO_TIME.getValue());
                        if (Objects.nonNull(time)) {
                            LocalTime occupiedTime = time.toLocalTime();
            ...
            connection.commit();
            return new PaginationResult<>(new ArrayList<>(result.values()), page, itemsOnPage, total);
        } catch (SQLException e) {
            getLogger().error(e);
            throw new RuntimeException(e);
        }
