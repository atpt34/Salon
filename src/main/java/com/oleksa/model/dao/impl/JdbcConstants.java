...
public interface JdbcConstants {
    ...
    String SC_SELECT_BY_ID = "SELECT * FROM schedule_t WHERE sc_id = ?";
	...
	String SC_UPDATE = "";
	String SC_DELETE_BY_ID = "DELETE FROM schedule_t WHERE sc_id = ?";
...
//	String RC_SELECT_ALL = "SELECT * FROM record_t INNER JOIN user_t ON us_id = rc_client_id INNER JOIN comment_t ON rc_comment_id = cm_id";
	String RC_SELECT_ALL = "SELECT * FROM record_t";
	String RC_SELECT_ALL_WITH_MASTER = "SELECT * FROM record_t INNER JOIN schedule_record_m2m ON rc_id = sr_rc_id "
			+ "INNER JOIN schedule_t ON sr_sc_id = sc_id INNER JOIN user_t ON sc_master_id = us_id "
			+ "WHERE rc_client_id = ?;";
...
//	String RC_SELECT_BY_SCHEDULE = "SELECT * FROM schedule_t INNER JOIN schedule_record_m2m ON sc_id = sr_sc_id INNER JOIN record_t ON rc_id = sr_rc_id WHERE sc_id = ?";
	...
	String RC_UPDATE = "UPDATE record_t SET rc_client_id = ?, rc_start_time = ?, rc_day = ?, rc_comment_id = ? WHERE rc_id = ?;";
	String RC_DELETE_BY_ID = "DELETE FROM record_t WHERE rc_id = ?;";
	
	String CM_ID = "cm_id";
	String CM_WORDS = "cm_words";
	String CM_STARS = "cm_stars";
	String CM_INSERT = "INSERT INTO comment_t (cm_words, cm_stars) VALUES (?, ?)";
	String CM_DELETE_BY_ID = "DELETE FROM comment_t WHERE cm_id = ?";
